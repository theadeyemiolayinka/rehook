...
Rehook deploys on Coolify or Dokploy using standard Docker Compose. Both platforms support Docker Compose deployments with persistent volumes, TLS termination, and automatic health checks.
...
3. Set the required environment variables in the platform UI:
   ...
   - `REHOOK_PUBLIC_BASE_URL`: your assigned HTTPS URL (e.g. `https://rehook.yourdomain.com`).
4. Deploy.
...
The named volume `rehook-data` is preserved across deploys. SQLite data survives container restarts and updates.
