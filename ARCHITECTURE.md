...
- Dispatch delivery instructions to connected agents. The server never
...
- Subscribe to one or more projects.
- Hold an explicit allowlist of local delivery targets per project.
...
2. Server validates the endpoint, enforces limits, captures the request, and
   stores the event. The provider gets an appropriate response immediately.
3. Server notifies any connected agents subscribed to the endpoint's project
   (optional, configurable).
4. Developer selects an event in the dashboard and triggers a replay to a
   connected agent.
5. Server sends a delivery instruction referencing the project and target.
